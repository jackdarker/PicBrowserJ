 ...
 * @author jkhome
 */
public class CmdAssignTags implements CmdInterface {

    
    protected DatPicture m_Picture=null;
    protected ArrayList<DatTag> m_Tags=null;
    protected Callable<CmdResult> m_PostAction;
    public CmdAssignTags(DatPicture Pic,ArrayList<DatTag> Tags, Callable<CmdResult> PostAction) {
        m_Picture = Pic;
        m_Tags = Tags;
    ...
    public void Redo() {
        try {
            if (m_Picture==null || m_Tags==null){
                ExecPostAction(new CmdResult(false,"not a picture"));
            } else {              
                m_Picture.setTags(m_Tags);
                ModelPictures.getInstance().SavePicture(m_Picture);
